...
const serviceWithUrlViaRegexSql =
`select id, name, url, comment from ${tables.services} where $1::text ~ url`

module.exports = function (pool, log, useRegex) {
  const serviceByUrlSql = (useRegex) ? serviceWithUrlViaRegexSql : serviceWithUrlSql
  ...
  async function getServiceWithUrl (url) {
    ...
    try {
      const result = await pool.query(serviceByUrlSql, [url])
      if (result.rows.length === 0) {
