...
module.exports.method = function (pool, log, Promise) {
  function * validateTGT (tgtId) {
    log.trace('validating tgt: %s', tgtId)
    try {
      const result = yield pool.query(getTgtSql, [tgtId])
      ...
      if (row.expires.getTime() < Date.now()) {
        ...
        yield pool.query(updateTgtSql, [tgtId])
  ...
  const coValidateTGT = Promise.coroutine(validateTGT)

  function * trackService (tgtId, stId) {
    log.trace('creating service tracking record: (%s, %s)', tgtId, stId)
    try {
      yield pool.query(trackStSql, [tgtId, stId])
      return true
    } catch (e) {
  ...
  const coTrackService = Promise.coroutine(trackService)

  function * genST (tgtId, expires, serviceId) {
    try {
      yield coValidateTGT(tgtId)
      const _expires = expires || joda.convert(joda.ZonedDateTime.now().plusMinutes(30)).toDate()
      const stId = `ST-${uuid.v4()}`
      const result = yield pool.query(insertStSql, [stId, tgtId, _expires, serviceId])
      log.trace('created service ticket: %s', stId)
      yield coTrackService(tgtId, stId)
      return {
        tid: stId,
  ...
  return Promise.coroutine(genST)
}
